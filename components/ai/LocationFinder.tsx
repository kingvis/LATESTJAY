...
const LocationFinder = () => {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!input.trim() || isLoading || !coords) return;
        ...
        try {
            const aiResponse = await getMapResponse(input, coords.lat, coords.lon);
            setMessages((prev) => [...prev, aiResponse]);
        } catch (error) {
            console.error(error);
            const errorMessage: ChatMessage = {
                sender: 'ai',
                text: "Sorry, I couldn't find location information right now. Please try again.",
            };
    ...
    if (locationState === 'PENDING' || locationState === 'DENIED') {
        return (
             <div className="text-center">
                <h3 className="text-xl font-bold text-foreground mb-2">Location Access Needed</h3>
                <p className="text-muted-foreground mb-4">To find places near you, please grant location access.</p>
                {locationState === 'DENIED' && <p className="text-red-500 mb-4">Location access was denied. Please enable it in your browser settings to use this feature.</p>}
                <button
    ...
    return (
        <div className="space-y-6 flex flex-col h-[65vh] max-h-[600px]">
            <div>
                <h3 className="text-xl font-bold text-foreground">Gig &amp; Jam Spot Finder</h3>
                <p className="text-muted-foreground">Find nearby open mic nights, recording studios, or music shops using your location.</p>
            </div>
            ...
            <div className="flex-1 p-4 overflow-y-auto scrollbar-hide space-y-4 bg-accent/50 rounded-lg border border-border">
                 {messages.length === 0 && (
                    <div className="text-center text-muted-foreground h-full flex items-center justify-center">
                        <p>Location enabled! Ask for nearby music spots...</p>
                    </div>
                )}
                    ...
                    <div key={index}>
                        ...
                        {msg.sources && msg.sources.length > 0 && (
                            <div className="mt-2 text-xs text-muted-foreground">
                                <strong className="text-foreground">Places Mentioned:</strong>
                                <ul className="list-disc list-inside">
                                    {msg.sources.map((source, i) => source.maps && (
                                        <li key={i}>
                                            <a href={source.maps.uri} target="_blank" rel="noopener noreferrer" className="text-secondary hover:underline">
                                                {source.maps.title}
                                            </a>
                                        </li>
                                    ))}
            ...
            <div className="">
                <form onSubmit={handleSendMessage} className="flex items-center space-x-2">
                    <input
                        ...
                        placeholder="e.g., Open mic nights near me?"
